...
| 写基金标书 | `grant` | research-scan → topic-selection → **novelty-check**(新颖性裁定+预注册) → grant-proposal → peer-review(自查) → render-pdf-doc | [references/pipeline-grant.md](references/pipeline-grant.md) |
| 写原创研究论文 | `paper` | **novelty-check**(前瞻性研究：新颖性+预注册，采数前) → deidentify(如含患者数据) → clinical-stats + data-analysis → nature-figure → write-paper → reference-check → humanize-academic → peer-review(自查/对抗红队) → render-docx | [references/pipeline-paper.md](references/pipeline-paper.md) |
