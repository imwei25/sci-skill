...
覆盖「脱敏 → 调研 → 选题 → 标书 → 综述 → 文献 → 深研 → 分析/统计 → 作图 → 写论文 → 去 AI 味 → 排版(PDF/Word) → 评审 → 查假引用」全链路的 **18 个 Agent 技能**（SKILL.md 格式，跨框架通用：OpenCode / OpenClaw / WorkBuddy / Claude Code 等）。Python 统一走**项目根 `.venv`**（正斜杠路径，bash/PowerShell 通用），不写死任何机器路径，`git clone` 到任意电脑即可用。
