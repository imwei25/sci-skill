...
- **未做/受限**：GB/T 7714 CSL 因本环境访问 GitHub/Gitee 拉取失败，未内置文件，改为在 render-docx 给出下载指引（citation-style-language/styles、zotero-chinese/styles、Gitee 镜像）。主控层（sci-pilot 路由 skill + AGENTS.md/workspace 状态）为审查报告建议项，本轮聚焦补 bug 与填 P0 缺口，主控留作后续。
