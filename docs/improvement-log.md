...
- 新增 `scripts/check_invariants.py`：机械校验改写前后**数字/引用标记/DOI/PMID/术语**集合 diff，把硬约束从声明变检查。实测：等价改写通过、篡改数字与引用被逐条揪出。
...
- **research-scan**：默认检索改 Europe PMC；"高被引/关键团队机构"强制走 **OpenAlex（pyalex 已装）** 聚合，查不到标"未核实"，堵住凭记忆报团队名的幻觉；补默认参数（近 5 年、30–50 篇、≤3000 字）。
